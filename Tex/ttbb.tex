...
Leptons are removed if they are separated from a jet by less than $\Delta R>0.4$.
...
%scaled to a common arbitrary integrated luminosity.
...
Two types of $b$-jet pairs are defined: one pair is build from the two $b$-jets with the highest $p_T$, called $maxPT$, and one pair from the two $b$-quarks which are closest in $\Delta R$, called $min \Delta R$ . The invariant mass of the b-jet pairs are shown in Figures~\ref{ttbb:MbbmaxPT} and~\ref{ttbb:MbbminDR}, the largest difference is seen between the Sherpa $\mathrm{t\bar{t}}$ and all other samples.
%
